...
<body class = "bg-light">
    ...
    <div class="container">
        ...
        <div class="container mt-4"> <!-- Main container for the course details -->
            <h1 class="mb-4">{{ $course->name }}</h1> <!-- Course name with bottom margin -->
        
            <div class="row"> <!-- Row for the layout -->
                <div class="col-12 text-center"> <!-- Full-width column for the image, centered -->
                    <img src="{{ asset($course->image) }}" alt="{{ $course->name }}" class="img-fluid mb-2" style="max-width: 70%;"> <!-- Centered image with specified max width -->
                </div>
                <div class="col-12 d-flex justify-content-between align-items-center"> <!-- Flex container for date and writer -->
                    <p class="text-muted mb-0">Published on: {{ $course->created_at->format('Y-m-d') }}</p> <!-- Date with no bottom margin -->
                    <p class="text-muted mb-0">Written by: {{ $course->writer->writerName }}</p> <!-- Writer name with no bottom margin -->
                </div>
                <div class="col-12 mt-3"> <!-- Column for the course detail -->
                    <p>{{ $course->detail }}</p> <!-- Course detail -->
                </div>
            </div>
        </div>
